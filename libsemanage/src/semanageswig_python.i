...
%typemap(argout) char** {
	$result = SWIG_Python_AppendOutput($result, SWIG_FromCharPtr(*$1), 0);
	free(*$1);
}
	...
	if ($result) {	
		...
		if (value >= 0) {
			...
			if (semanage_array2plist($1, (void**) *$3, *$4,
				...
				$result = SWIG_Python_AppendOutput($result, plist, 0);
		}
	}
}
...
%typemap(argout) semanage_module_info_t ** {
	$result = SWIG_Python_AppendOutput($result, SWIG_NewPointerObj(*$1, $*1_descriptor, 0), 0);
}
